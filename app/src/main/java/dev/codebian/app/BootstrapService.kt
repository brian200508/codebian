...
class BootstrapService : Service() {
    ...
    /**
     * Backstop for every `scope.launch { ... }` below: [runBootstrap] and
     * [updateCodeServer] already catch their own errors and report them via
     * [BootstrapManager.Error], but the on-demand sync paths triggered from
     * [onStartCommand] (`ACTION_SYNC_*`) and the companion object's
     * `request*Sync` functions did not -- any exception there (network
     * hiccup, proot failure, etc.) was previously left to propagate out of
     * the coroutine uncaught, which crashes this process entirely rather
     * than just showing an error state. Installing this handler on the
     * scope itself means *any* uncaught exception from *any* launch here
     * is routed here instead of crashing, without needing every call site
     * to remember its own try/catch.
     */
    private val exceptionHandler = CoroutineExceptionHandler { _, throwable ->
        Log.e("CoDebianBootstrap", "Unhandled error in background bootstrap task", throwable)
        BootstrapManager.update(BootstrapState.Error(throwable.message ?: throwable.toString()))
    }
    private val job = Job()
    ...
    private fun downloadAndExtractRootfs() {
        ...
        val digest = obtainRootfsArchive(archive)

        BootstrapManager.update(BootstrapState.VerifyingDownload)
        if (!DockerRegistryClient().verifyDigest(archive, digest)) {
            archive.delete()
            error("Downloaded rootfs failed SHA-256 verification against $digest -- refusing to extract/execute a corrupted or tampered image")
        }

        BootstrapManager.update(BootstrapState.ExtractingRootfs)
        extractRootfsArchive(archive)
        archive.delete()
    }

    /**
     * Fills [archive] with the Debian rootfs layer's bytes and returns the
     * `sha256:<hex>` digest it's expected to match, preferring the copy
     * bundled into the APK at build time by `scripts/fetch-assets.*` (see
     * [RemoteAssets.ROOTFS_ASSET_NAME]) over a live Docker Hub pull.
     *
     * This is what makes the app itself immune to the exact failure mode
     * that prompted this whole bundling effort: previously *every* first
     * launch depended on Docker Hub being reachable and the pinned
     * `trixie` tag still resolving the same way, so any registry outage or
     * upstream change broke already-installed copies of the app, not just
     * future builds. With the asset present, first run needs no network
     * access for the rootfs at all; an already-published APK keeps working
     * exactly as it did on the day it was built, indefinitely.
     */
    private fun obtainRootfsArchive(archive: File): String {
        val digestAsset = "${RemoteAssets.ROOTFS_ASSET_NAME}.sha256"
        return try {
            val bundledDigest = assets.open(digestAsset).bufferedReader().use { it.readText() }.trim()
            BootstrapManager.update(BootstrapState.Downloading("Debian rootfs (bundled)", 0))
            assets.open(RemoteAssets.ROOTFS_ASSET_NAME).use { input ->
                FileOutputStream(archive).use { output -> input.copyTo(output) }
            }
            BootstrapManager.update(BootstrapState.Downloading("Debian rootfs (bundled)", 100))
            "sha256:$bundledDigest"
        } catch (_: java.io.FileNotFoundException) {
            // Not bundled (fetch-assets wasn't run for this build, e.g. a
            // quick local dev build) -- fall back to the original live pull.
            downloadRootfsFromRegistry(archive)
        }
    }

    private fun downloadRootfsFromRegistry(archive: File): String {
    ...
    private fun extractRootfsArchive(archive: File) {
    ...
    private fun ensureCodeServerInstalled() {
        ...
        if (installBundledCodeServer()) return
    ...
    /**
     * Installs code-server fully offline from the exact `.deb` bundled into
     * the APK at build time (see [RemoteAssets.CODE_SERVER_ASSET_NAME] /
     * `scripts/fetch-assets.*`), sha256-verified against the pinned digest
     * in [RemoteAssets.CODE_SERVER_DEB_SHA256] before install. This is what
     * makes first-run install deterministic and independent of
     * `code-server.dev/install.sh` (which always fetches whatever the
     * *latest* GitHub release happens to be at curl-time) staying
     * reachable. Returns false (falling back to the old online path in
     * [ensureCodeServerInstalled]) if this build wasn't bundled with one --
     * e.g. a quick local dev build where `fetch-assets` wasn't run.
     */
    private fun installBundledCodeServer(): Boolean {
        val localDeb = File(cacheDir, "code-server.deb")
        try {
            assets.open(RemoteAssets.CODE_SERVER_ASSET_NAME).use { input ->
                FileOutputStream(localDeb).use { output -> input.copyTo(output) }
            }
        } catch (_: java.io.FileNotFoundException) {
            return false
        }
        try {
            check(DockerRegistryClient().verifyDigest(localDeb, RemoteAssets.CODE_SERVER_DEB_SHA256)) {
                "Bundled code-server .deb failed SHA-256 verification against ${RemoteAssets.CODE_SERVER_DEB_SHA256} -- refusing to install a corrupted or tampered package"
            }
            val guestRelativePath = "root/.codebian-code-server.deb"
            localDeb.copyTo(File(proot.extractRootfsTarget(), guestRelativePath), overwrite = true)
            val exit = proot.runInRootfs(
                listOf(
                    "bash", "-lc",
                    "dpkg -i /$guestRelativePath || (apt-get update && apt-get install -f -y); " +
                        "rm -f /$guestRelativePath"
                )
            )
            check(exit == 0) { "bundled code-server install failed (exit $exit) -- see logcat tag CoDebianProot" }
            return true
        } finally {
            localDeb.delete()
        }
     ...
     * pull in newer releases. Deliberately kept as the one remaining
     * always-online path (the user has to explicitly ask for "latest",
     * separate from the bundled/pinned version used at first install).
     */
    ...
    /**
     * Points the rootfs's apt sources at a fixed snapshot.debian.org
     * timestamp ([RemoteAssets.APT_SNAPSHOT_TIMESTAMP]) instead of the
     * live `trixie` suite, so every `apt-get install` below (git, python3,
     * nodejs, openssh-server, sudo, ...) resolves to the exact same
     * package versions forever -- snapshot.debian.org keeps every version
     * it has ever served indefinitely, unlike a live suite (which drops
     * old package versions as new ones are uploaded) or a random
     * third-party mirror (which can vanish outright, as proot's mirror
     * does on every new release). Idempotent via a marker file; safe to
     * call on every [runBootstrap], including for installs that predate
     * this feature.
     *
     * `[check-valid-until=no]` plus the matching apt.conf override are
     * required because a snapshot's `Release` file's `Valid-Until` field
     * is (deliberately) always in the past by the time anyone builds
     * against it months/years later -- without this, apt would refuse to
     * use the snapshot at all once "expired".
     *
     * Uses plain `http://`, not `https://`, deliberately: the minimal
     * Debian rootfs layer has no guaranteed up-to-date CA bundle at the
     * point this runs (this is the very first thing `runBootstrap` does
     * apt-wise, before `ca-certificates` has been confirmed installed/
     * refreshed), so an HTTPS fetch can fail with
     * "SSL routines::certificate verify failed" depending on exactly
     * which snapshot the rootfs layer itself was built from. This is safe
     * apt package integrity never depends on transport-level TLS -- every
     * Release/InRelease file is GPG-signed and apt verifies that
     * signature (against /etc/apt/trusted.gpg.d/) regardless of http vs
     * https, so a plain-http snapshot mirror can't serve tampered
     * packages undetected; TLS here would only additionally hide the
     * transfer from a network eavesdropper, which isn't a concern for
     * public Debian package downloads.
     */
    private fun pinAptToSnapshot() {
        val marker = File(proot.extractRootfsTarget(), "root/.codebian-apt-pinned")
        val snapshotUrl = "http://snapshot.debian.org/archive/debian/${RemoteAssets.APT_SNAPSHOT_TIMESTAMP}/"
        // The marker's *content* is the URL it pinned to, not just its
        // presence, so that changing snapshotUrl (e.g. the https -> http
        // scheme fix, or a future APT_SNAPSHOT_TIMESTAMP bump) re-runs this
        // on already-bootstrapped installs instead of silently keeping a
        // stale/broken sources.list forever.
        if (marker.exists() && marker.readText().trim() == snapshotUrl) return
        val exit = proot.runInRootfs(
            listOf(
                "bash", "-lc",
                "set -e; " +
                    // Recent Debian docker images default to the deb822
                    // /etc/apt/sources.list.d/debian.sources format -- remove
                    // it so it can't conflict with the legacy-format
                    // sources.list written below (both formats are read by
                    // apt if both are present, causing duplicate/conflicting
                    // 'main' sources).
                    "rm -f /etc/apt/sources.list.d/debian.sources; " +
                    "printf 'deb [check-valid-until=no] %s ${RemoteAssets.DEBIAN_TAG} main\\n' '$snapshotUrl' > /etc/apt/sources.list; " +
                    "mkdir -p /etc/apt/apt.conf.d; " +
                    "printf 'Acquire::Check-Valid-Until \"false\";\\n' > /etc/apt/apt.conf.d/99codebian-snapshot; " +
                    "printf '%s' '$snapshotUrl' > /root/.codebian-apt-pinned"
            )
        )
        check(exit == 0) { "pinning apt sources to snapshot.debian.org failed (exit $exit) -- see logcat tag CoDebianProot" }
    }
     ...
     * typically far behind current LTS -- pinned to an explicit major
     * version ([RemoteAssets.NODEJS_MAJOR_VERSION]) rather than
     * `setup_lts.x`, which silently resolves to whatever is current LTS
     * *at curl-time* and could pull in a different, untested major version
    ...
    private fun ensureBundledToolsInstalled() {
        ...
        ensureAptPackageInstalled(
            ...
            "apt-get update; apt-get install -y curl ca-certificates gnupg; " +
                "curl -fsSL https://deb.nodesource.com/setup_${RemoteAssets.NODEJS_MAJOR_VERSION}.x | bash -; apt-get install -y nodejs",
            // Marker content -- not just its presence -- is checked below,
            // so bumping NODEJS_MAJOR_VERSION (e.g. 22 -> 24) re-runs this
            // install/upgrade on already-bootstrapped installs instead of
            // leaving them stuck on whatever major version they first
            // installed forever.
            versionTag = RemoteAssets.NODEJS_MAJOR_VERSION,
        )
    }

    private fun ensureAptPackageInstalled(tool: String, markerPath: String, installCommand: String, versionTag: String = "") {
        val marker = File(proot.extractRootfsTarget(), markerPath)
        if (marker.exists() && marker.readText().trim() == versionTag) return
        BootstrapManager.update(BootstrapState.InstallingBundledTools(tool))
        val exit = proot.runInRootfs(
            listOf("bash", "-lc", "set -e; $installCommand; printf '%s' '$versionTag' > /$markerPath")
        )
